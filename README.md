...
**Status: v0.1.1 local preview implementation.** Local learning, progress, checks, and optional tutoring now include untracked overview/reference pages and draft-only activity-tool handoffs. This describes source behavior, not a claim that v0.1.1 has been published or installed. For current validation, check the test suite and actual run results; historical v0.1.0 validation is recorded below. Native notebook maintenance, real tutor interactions, and general Agent handoffs still require user acceptance testing. A CI configuration alone is not evidence of a passing run. See [CHANGELOG.md](CHANGELOG.md) for scope.
...
Progress reset cancels outstanding checks for that course. It does **not** change lessons, notebooks, answers, outputs, or cloud resources. Output clearing does **not** restore a pristine exercise. The extension has no built-in baseline restore, automatic source reversion, or cloud teardown feature; **Revert unit** is a draft handoff, not a restore engine. Use the extension's **Reset progress** separately if you also want to reset the selected unit's learning record.
...
- Restricted Mode permits reading, navigation, manual completion, opening resources, and progress reset. Checks, tutor sharing, activity-tool drafts, and output edits require a trusted workspace. Native notebook execution has its own host/provider controls.
...
- `npm test` compiles tests, type-checks, lints, builds the extension, and runs core, rendering/status, runner/check-result, course-page, activity-tool, and portable UI-action suites. UI-action tests exercise compiled panels with the VS Code boundary mocked; no real host is launched. Check the current suite and run output rather than relying on a fixed test count.
